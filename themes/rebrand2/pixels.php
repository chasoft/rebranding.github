	...
	<div id="user-content" class="col-md-8">
		...
		<div class="main-content panel panel-default panel-body">
			<?php if (!$this->isTeam() || ($this->isTeam() && $this->teamPermission("pixels.create"))) : ?>
				...
				<form action="<?php echo Main::href("user/pixels") ?>" role="form" method="post" id="pixelsform" name="pixelsform">
					<div class="row">
						<div class="col-md-4">
						...
						<div class="col-md-4">
							...
							<input type="text" value="" name="name" class="form-control" placeholder="e.g. <?php echo e("Shopify Campaign") ?>" required autofocus />
						</div>
						<div class="col-md-4">
							<label for="tag" class="label-control"><?php echo e("Pixel Tag") ?></label>
							<input type="text" value="" name="tag" class="form-control" placeholder="e.g. <?php echo e("Numerical or alphanumerical values only") ?>" minlength="5" required />
						</div>
					</div>
				...
				<script>
					jQuery.validator.addMethod("gtmfirst", function(value, element) {
						if ($(".chosen-single span").text() == "Google Tag Manager") {
							if (/GTM/.test(value)) {
								return true;
							} else {
								return false;
			...
			<?php if (is_array($gtmpixel) && !empty($gtmpixel)) : ?>
				...
				<?php foreach ($gtmpixel as $id => $gtm) : ?>
					<form action="<?php echo Main::href("user/pixels/save?id={$id}") ?>" method="post" id="gtm_pixels_form" name="gtm_pixels_form">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Name") ?></label>
									<input type="text" value="<?php echo $gtm["name"] ?>" name="gtmpixel[name]" class="form-control" placeholder="e.g. Shopify Campaign" required />
								</div>
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Tag") ?></label>
									<input type="text" value="<?php echo $gtm["tag"] ?>" name="gtmpixel[tag]" class="form-control" minlength="5" placeholder="e.g. 1234567890123456" required />
								</div>
								<div class="col-sm-4">
					...
					<script>
						jQuery.validator.addMethod("gtm", function(t, e) {
							return !!/GTM/.test(t)
						}, "Please enter a valid Tag."), $("#gtm_pixels_form").validate({
							rules: {
								"gtmpixel[tag]": {
									gtm: !0
								}
			...
			<?php if (is_array($fbpixel) && !empty($fbpixel)) : ?>
				...
				<?php foreach ($fbpixel as $id => $fb) : ?>
					<form action="<?php echo Main::href("user/pixels/save?id={$id}") ?>" method="post" id="fb_pixels_form" name="fb_pixels_form">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Name") ?></label>
									<input type="text" value="<?php echo $fb["name"] ?>" name="fbpixel[name]" class="form-control" placeholder="e.g. Shopify Campaign" required />
								</div>
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Tag") ?></label>
									<input type="number" value="<?php echo $fb["tag"] ?>" name="fbpixel[tag]" class="form-control" placeholder="e.g. 1234567890123456" maxlength="20" required />
								</div>
								<div class="col-sm-4">
									<div class="btn-group addmargin">
										<?php if (!$this->isTeam() || ($this->isTeam() && $this->teamPermission("pixels.edit"))) : ?>
			...
			<?php if (is_array($adwordspixel) && !empty($adwordspixel)) : ?>
				...
				<?php foreach ($adwordspixel as $id => $adw) : ?>
					<form action="<?php echo Main::href("user/pixels/save?id={$id}") ?>" method="post" id="adwords_pixels_form" name="adwords_pixels_form">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Name") ?></label>
									<input type="text" value="<?php echo $adw["name"] ?>" name="adwordspixel[name]" class="form-control" placeholder="e.g. Shopify Campaign" required />
								</div>
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Tag") ?></label>
									<input type="text" value="<?php echo $adw["tag"] ?>" name="adwordspixel[tag]" class="form-control" maxlength="40" placeholder="e.g. AW-12345678901/ABCDEFGHIJKLMOPQRST" required />
								</div>
								<div class="col-sm-4">
									<div class="btn-group addmargin">
										<button type="submit" class="btn btn-default"><i class="fa fa-save"></i></button>
			...
			<?php if (is_array($linkedinpixel) && !empty($linkedinpixel)) : ?>
				...
				<?php foreach ($linkedinpixel as $id => $lkd) : ?>
					<form action="<?php echo Main::href("user/pixels/save?id={$id}") ?>" method="post" id="linkedin_pixels_form" name="linkedin_pixels_form">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Name") ?></label>
									<input type="text" value="<?php echo $lkd["name"] ?>" name="linkedinpixel[name]" class="form-control" placeholder="e.g. Shopify Campaign" required />
								</div>
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Tag") ?></label>
									<input type="text" value="<?php echo $lkd["tag"] ?>" name="linkedinpixel[tag]" class="form-control" maxlength="10" placeholder="e.g. 123456" required />
								</div>
								<div class="col-sm-4">
									<div class="btn-group addmargin">
										<button type="submit" class="btn btn-default"><i class="fa fa-save"></i></button>
			...
			<?php if (is_array($twitterpixel) && !empty($twitterpixel)) : ?>
				...
				<?php foreach ($twitterpixel as $id => $tw) : ?>
					<form action="<?php echo Main::href("user/pixels/save?id={$id}") ?>" method="post" id="twitter_pixels_form" name="twitter_pixels_form">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Name") ?></label>
									<input type="text" value="<?php echo $tw["name"] ?>" name="twitterpixel[name]" class="form-control" placeholder="e.g. Shopify Campaign" required />
								</div>
								<div class="col-sm-4">
									<label class="control-label"><?php echo e("Tag") ?></label>
									<input type="text" value="<?php echo $tw["tag"] ?>" name="twitterpixel[tag]" class="form-control" maxlength="15" placeholder="e.g. 123456789" required />
								</div>
								<div class="col-sm-4">
									<div class="btn-group addmargin">
										<button type="submit" class="btn btn-default"><i class="fa fa-save"></i></button>
